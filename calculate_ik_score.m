...
% Pennsylvania. It tests the full inverse kinematics for the PUMA 260
% for one team, calculating a score that reflects overall IK
% performance.
...
% Define the name of your the inverse kinematics function you want to
% test. The @ symbol in front of the function name creates a function
% handle, so that we can call the function indirectly.  We do this so
% that you can quickly change the IK function that you want to test,
% without having to change it in many places down below in the code.
puma_ik = @team119_puma_ik;
...
% Set the first test to be the home pose.
...
% Set the second test to be the zero pose.
T60zero = puma_t60_kuchenbe(0,0,0,0,0,0);
ox_history(2) = T60zero(1,4);
oy_history(2) = T60zero(2,4);
oz_history(2) = T60zero(3,4);
phi_history(2) = 0;
theta_history(2) = 0;
psi_history(2) = 0;

% Set number of manually specified tests from above.
nManualTests = 2; % Set this to zero if you don't want to test the home pose and the zero pose.
...
% Reset the random number generator to its default startup settings,
% so that rand produces the same random numbers every time.
rng('default')

% Choose random values for all joint angles.
% rand fills the matrix by columns, so we make those the test cases
% and transpose it so that the first test is always the same, as is
% the second, etc.
...
%% Calculate IK score.
% Calculate a score that reflects the performance of this team's
% inverse kinematics function.

% Fill the score matrix with zeros.  The score matrix has eight
% columns for the eight possible solutions, each of which have six
% entries for each aspect of the score.
    ...
    if (s(1) ~= 6)
        ...
        for j = 1:s2(2)
            ...
            if(any(isnan(thetas(:,j))))
                % All situations tested are reachable, so give this
                % solution a -3 score.
                scores(i,j,:) = -3;
            else
                
                % Push the returned joint angles though the forward
                % kinematics of the robot.
                T60 = puma_t60_kuchenbe(thetas(1,j), thetas(2,j), thetas(3,j), ...
                    thetas(4,j), thetas(5,j), thetas(6,j));
                
                % Calculate x, y, and z position scores as one minus
                % the absolute distance error in inches in that
                % direction.  A correct solution will have zero error,
                % so the score will be 1.  Worse solutions have more
                % negative scores.
                scores(i,j,1) = 1 - abs(T60(1,4) - ox_history(i));
                scores(i,j,2) = 1 - abs(T60(2,4) - oy_history(i));
                scores(i,j,3) = 1 - abs(T60(3,4) - oz_history(i));
                
                % Calculate x, y, and z orientation scores as -9 plus
                % ten times the dot product between the desired final
                % frame unit vector and athe actual final frame unit
                % vector.  A correct solution will have a dot product
                % of one, so the score will be 1.  Worse solutions
                % will have more negative scores.
