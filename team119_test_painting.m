%% team119_test_painting.m
%
% This Matlab script is part of the starter code for the simulated
% light painting part of Project 1 in MEAM 520 at the University of
% Pennsylvania. It tests a team's inverse kinematics code for their
% light painting.
...
% Clear the console, so you can more easily find any errors that may
% occur.
clc
...
% To test the positions and orientations from the team's light
% painting, we call the team's light painting code.
...
for i = 1:length(t)
    [~, ox_history(i), oy_history(i), oz_history(i), phi_history(i), ...
        ...
        b_history(i)] = team119_get_poc(t(i));
end
...
for i = 1:length(ox_history)
    ...
    allthetas = team119_puma_ik(ox, oy, oz, phi, theta, psi);
    
    if (i == 1)
        % Pass in the home position as the current position.
        thetas_history(i,:) = team119_choose_solution(allthetas, [0 0 0 0 -pi/2 0]')';
    else        
        % Choose the solution to show.
        thetas_history(i,:) = team119_choose_solution(allthetas, thetas_history(i-1,:)')';
    end
